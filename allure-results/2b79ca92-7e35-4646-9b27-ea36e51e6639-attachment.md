...
- Location: tests\accountLogin.spec.ts:33:9

# Error details

```
Error: locator.isVisible: Error: strict mode violation: getByRole('link', { name: 'My Account' }) resolved to 2 elements:
    1) <a title="My Account" data-toggle="dropdown" class="dropdown-toggle" href="http://localhost/opencart/upload/index.php?route=account/account">…</a> aka getByRole('link', { name: ' My Account' })
    2) <a href="http://localhost/opencart/upload/index.php?route=account/account">My Account</a> aka getByRole('link', { name: 'My Account', exact: true })

Call log:
    - checking visibility of getByRole('link', { name: 'My Account' })
...
```ts
  ...
  21  |         this.lnkMyAccount = page.getByRole('link', { name: 'My Account' });
  22  |         this.lnkRegister = page.getByRole('link', { name: 'Register' });
