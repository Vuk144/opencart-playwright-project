...
```ts
  ...
  40 |     const logoutPage = await myAccountPage.clickLogout();
  41 |     let isLogoutPageVisible = await logoutPage.isContinueButtonVisible();
> 42 |     expect(isLogoutPageVisible).toBeTruthy();
     |                                 ^ Error: expect(received).toBeTruthy()
